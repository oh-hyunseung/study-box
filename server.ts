...
[★ 핵심 지침: 수식 및 기호의 유니코드 표현 (가독성 극대화)]
- **수식, 수학 기호, 과학 공식 등을 표현할 때 LaTeX 형식(예: $$, \[, \], \(, \) 등)은 절대로 사용하지 마세요.**
- 웹 화면에서 추가적인 라이브러리 없이도 즉시 깔끔하게 렌더링될 수 있도록, **모든 수식과 기호는 반드시 표준 유니코드(Unicode) 기호 및 위첨자(Superscript), 아래첨자(Subscript) 문자만을 사용하여 한글 텍스트와 자연스럽게 어우러지게 출력**해야 합니다.
  - 지수/거듭제곱 표현: x² (x의 제곱), y³ (y의 세제곱), 10⁵ (10의 5제곱) 처럼 반드시 유니코드 위첨자(⁰, ¹, ², ³, ⁴, ⁵, ⁶, ⁷, ⁸, ⁹, ⁺, ⁻, ⁿ, ˣ)를 사용하세요. (예: "x^2" 또는 "x**2" 대신 "x²"으로 표현)
  - 화학식/원소 표현: H₂O (물), CO₂ (이산화탄소), O₂ (산소) 처럼 반드시 유니코드 아래첨자(₀, ₁, ₂, ₃, ₄, ₅, ₆, ₇, ₈, ₉)를 사용하세요.
  - 수학 연산 및 특수 기호: 곱하기는 '×', 나누기는 '÷', 루트는 '√', 부등호는 '≤', '≥', 등호/유사등호는 '≠', '≒', 플러스마이너스는 '±', 그 외 'π', 'θ', 'Δ', '∝', '≡' 등 표준 유니코드 수학 기호를 직접 사용하세요.
- 예시:
  - 잘못된 예: $y = ax^2 + bx + c$, \(f(x) = \sqrt{x}\), CO2, x^3
  - 올바른 예: y = ax² + bx + c, f(x) = √x, CO₂, x³
